...
if(isset($_GET['employeeId'])){
    ...
    $result = $Employee->getAllEmployeesSalaryReportPerMonth($employeeId);
}
...
// Row 1
if(isset($_GET['employeeId'])){
    $pdf->Cell(0,6,strtoupper($result[0]['FULL_NAME'].' SALARY'),0,1,'C');
} else {
    $pdf->Cell(0,6,strtoupper('EMPLOYEES SALARY '),0,1,'C');
}
...
if(isset($_GET['employeeId'])){
    ...
    foreach($result as $key => $value)
    {
        ...
        $pdf->Cell(70,7,$value['FULL_NAME'],1,0,'C');
        $pdf->Cell(37,7,$value['TOTAL_HOURS_WORKED'],1,0,'C');
        $pdf->Cell(20,7,$value['SALARY_RATE']." Pesos",1,0,'C');
        $pdf->Cell(20,7,$value['TOTAL_SALARY']." Pesos",1,0,'C');
        $pdf->Cell(30,7,$value['MONTH'],1,1,'C');
        $i++;
    }
} else {
    for ($i = 1; $i <= 10; $i++)
    {
        $pdf->Cell(13,7,$i,1,0,'C');
        $pdf->Cell(70,7,"value",1,0,'C');
        $pdf->Cell(37,7,"value",1,0,'C');
